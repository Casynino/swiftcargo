...
/**
 * THE COMBINED INVOICE, FROM THE LINK IN THE MERGED NOTIFICATION.
 *
 * No session, same as a single consignment's invoice — the key in the link
 * stands in for one, signed over the group of invoice ids so it cannot be
 * composed for bills somebody only guessed at. See
...
export async function GET(
  ...
  if (!trackKeyValid(key, url.searchParams.get("k"))) {
