    ...
    <div className="space-y-2">
      ...
      {open ? (
        <form action={damageAction} className="space-y-2 rounded-md border bg-secondary/40 p-2">
          <input type="hidden" name="boxId" value={boxId} />
          <Input name="note" placeholder="What is wrong — crushed, wet, opened…" required minLength={3} />
          <Input name="photo" type="file" accept="image/*" capture="environment" multiple />
          <SubmitButton size="sm" pendingLabel="Saving…">Save damage report</SubmitButton>
